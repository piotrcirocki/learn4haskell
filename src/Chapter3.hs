...
bookLordOfTheRings :: Book
bookLordOfTheRings =
  BookProduct
    { bookName = "Lord Of The Rings",
      numberOfPages = 900,
      authors = ["Ronald Tolkien", "Son Of Tolkien"]
    }
{- |
=⚔️= Task 2
...
-}
data Monster = MonsterCreature
  { monsterHealth :: Int,
    monsterAttack :: Int,
    monsterGold :: Int
  }

data Knight = KnightFighter
  { knightHealth :: Int,
    knightAttack :: Int,
    knightGold :: Int
  }
...
fight (MonsterCreature mH mA mG) (KnightFighter kH kA kG) =
  let knightHit = mH - kA
      monsterHit = kH - mA
   in if knightHit > 0 then
        if monsterHit > 0 then kG else -1
      else  kG + mG

createFight :: Int -> Int -> Int
createFight monHealth knHealth =
  let monster =
        MonsterCreature
      ...
      knight =
        KnightFighter
          { knightHealth = knHealth,
            knightAttack = 10,
            knightGold = 10
          }
   in fight monster knight
...
-}
data Castle = CastleName String deriving (Show)

data ChurchOrLib = Church | Library deriving (Show)

data People = One | Two | Three | Four deriving (Show)

data Walls = WallsExists Castle deriving (Show)

data House = LivingHouse {peopleInHouse :: People} deriving (Show)

data City = LivingCity
  { castleInCity :: Maybe Castle,
    walls :: Maybe Walls,
    churchOrLib :: ChurchOrLib,
    houses :: [House]
  }
  deriving (Show)

buildCastle :: City -> Castle -> City
buildCastle (LivingCity castleCity castleWalls churchOrLibCastle housesInCity) (CastleName newCastle) =
  let castle = CastleName newCastle
   in case castleCity of
        Nothing -> LivingCity castleCity castleWalls churchOrLibCastle housesInCity
        _ -> LivingCity (Just castle) (Just (WallsExists castle)) churchOrLibCastle housesInCity

testBuildCastle :: City
testBuildCastle =
  let exampleCastle = CastleName "Castle"
      anotherCastle = CastleName "Castle2"
      wallsExample = WallsExists exampleCastle
      peoplesExample = One
      exampleCity =
        LivingCity
          { castleInCity = Just exampleCastle,
            walls = Just wallsExample,
            churchOrLib = Church,
            houses = [LivingHouse {peopleInHouse = peoplesExample}]
          }
   in buildCastle exampleCity anotherCastle

buildHouse :: People -> House
buildHouse p =
  let newlyBuiltHouse =
        LivingHouse
          { peopleInHouse = p
...
numberOfPeople :: [House] -> Int
numberOfPeople [] = 0
numberOfPeople arr = go arr 0
  where
    go :: [House] -> Int -> Int
    go [] c = c
    go (x : xs) count = case peopleInHouse x of
      One -> go xs (count + 1)
      Two -> go xs (count + 2)
      Three -> go xs (count + 3)
      Four -> go xs (count + 4)
...
buildWalls (LivingCity cCastleInCity cWalls cChurchOrLib cHouses) =
  let defaultCity =
        LivingCity
          { castleInCity = cCastleInCity,
            walls = cWalls,
            churchOrLib = cChurchOrLib,
            houses = cHouses
          }
      currentCastleName = case cCastleInCity of
        Nothing -> ""
        Just (CastleName name) -> name
      castleToBuildWall = CastleName currentCastleName
      wallsToBeBuilt = WallsExists castleToBuildWall
      numberOfAllPeople = numberOfPeople cHouses
      newlyBuiltCity =
        LivingCity
          { castleInCity = cCastleInCity,
            walls = Just wallsToBeBuilt,
            churchOrLib = cChurchOrLib,
            houses = cHouses
          }
   in case cCastleInCity of
        Nothing -> defaultCity
        Just _ -> if numberOfAllPeople >= 10 then newlyBuiltCity else defaultCity

buildWallsEx1 :: City
buildWallsEx1 =
  let sampleCastle = CastleName "SomeCastle"
      samplePeople = Four
      sampleCity =
        LivingCity
          { castleInCity = Just sampleCastle,
            walls = Nothing,
            churchOrLib = Church,
            houses = [LivingHouse {peopleInHouse = samplePeople}]
          }
   in buildWalls sampleCity
      ...
      sampleCity =
        LivingCity
          { castleInCity = Just sampleCastle,
            walls = Nothing,
            churchOrLib = Church,
            houses = [LivingHouse {peopleInHouse = Four}, LivingHouse {peopleInHouse = Four}, LivingHouse {peopleInHouse = Four}]
          }
   in buildWalls sampleCity
{-
=🛡= Newtypes
...
-}
newtype Attack = Att Int deriving (Show)
newtype Strength = Str Int deriving (Show)
newtype Armor = Arm Int deriving (Show)
newtype Health = Hea Int deriving (Show)
newtype Dextertity = Dex Int deriving (Show)
newtype Damage = Dam Int deriving (Show)
newtype Defense = Def Int deriving (Show)

data Player = Player
  { playerHealth :: Health,
    playerArmor :: Armor,
    playerAttack :: Attack,
    playerDexterity :: Dextertity,
    playerStrength :: Strength
  } deriving (Show)

calculatePlayerDamage :: Attack -> Strength -> Damage
calculatePlayerDamage (Att attack) (Str strength) = Dam (attack +  strength)

calculatePlayerDefense :: Armor -> Dextertity -> Defense
calculatePlayerDefense (Arm armor) (Dex dexterity) = Def (armor * dexterity)

calculatePlayerHit :: Damage -> Defense -> Health -> Health
calculatePlayerHit (Dam damage) (Def defense) (Hea health) = Hea (health + defense - damage)
...
hitPlayerEx =
  let player1 = Player{
        ...
        playerArmor = Arm 3,
        playerAttack = Att 6,
        playerDexterity = Dex 20,
        playerStrength = Str 40
               }
      player2 = Player{
        playerHealth = Hea 16,
        playerArmor = Arm 8,
        playerAttack = Att 45,
        playerDexterity = Dex 55,
        playerStrength = Str 42
                 }
  in hitPlayer player1 player2
...
data DragonsLair tr dr = DragonsLairCave{
                    treasureLair :: MaybeT tr,
                    dragon :: Dragon dr
                                  } deriving (Show)
dragonsLairEx :: DragonsLair String String
dragonsLairEx =
  let treasureGold tre = JustTreasure tre
      dragonNew pwr = Dragon{magicPower = pwr}
      dragonsLairCave tt dd = DragonsLairCave{
                          treasureLair = tt,
                          dragon = dd
                        }
  in dragonsLairCave (treasureGold "gold") (dragonNew "FireBall")
{-
=🛡= Typeclasses
...
-}
newtype Gold = GoldenCoins Int deriving (Show)
...
instance Append Gold where
  ...
  append (GoldenCoins gold1) (GoldenCoins gold2) = GoldenCoins(gold1 + gold2)
...
instance (Append a) => Append (Maybe a) where
  ...
  append (Just b) (Just c) = Just append <*> Just b <*> Just c
  append Nothing (Just c) = Just c
  append (Just b) Nothing = Just b
  append Nothing Nothing = Nothing

appendGoldEx1 :: Gold
appendGoldEx1 =
  let gold1 = GoldenCoins 2
      gold2 = GoldenCoins 3
  in append gold1 gold2

appendGoldEx2 :: [Gold]
appendGoldEx2 =
  let coins1 = [GoldenCoins 3, GoldenCoins 6]
      coins2 = [GoldenCoins 6]
  in append coins1 coins2

appendGoldEx3 :: Maybe Gold
appendGoldEx3 =
  let coins1 = Just (GoldenCoins 6)
      coins2 = Just (GoldenCoins 7)
  in append coins1 coins2

appendGoldEx4 :: Maybe Gold
appendGoldEx4 =
  let coins1 = Just (GoldenCoins 4)
      coins2 = Nothing
  in append coins1 coins2

appendGoldEx5 :: Maybe Gold
appendGoldEx5 =
  let coins1 = Nothing
      coins2 = Nothing
  in append coins1 coins2
